...
namespace interface {
    class CR1_PRO_WBC_SimInterface : public RobotInterface {
    private:
        double run_time_ = 0;
        double run_time_frist = 0;
    ...
    public:
        ...
        virtual void Start() {
            ...
            sim_thread_ = std::thread(std::bind(&CR1_PRO_WBC_SimInterface::ReceiveRobotData, this));
            send_thread_ = std::thread(std::bind(&CR1_PRO_WBC_SimInterface::SendJointCmd, this));
        ...
        void ReceiveRobotData() {
            ...
            while (start_thread_flag_) {
                ...
                if (run_time_frist == 0)
                    run_time_frist = data[0];
                run_time_ = data[0] - run_time_frist;
                // std::cout<<"run_time_"<<run_time_<<"run_time_frist"<<run_time_frist<<"data[0]"<<data[0]<<std::endl;
                ...
                // std::cout<<"run_time_"<<run_time_<<std::endl;
                // std::cout<<"joint 6"<<joint_pos_[6]<<std::endl;
                // if (last_join_pos.isApprox(joint_tau_, 1e-3)) {
                //         // joint_pos_ 是新数据
                //         std::cout<<"dadfafaffgfgbgfbgf"<<std::endl;
                //     }
                // last_join_pos = joint_tau_;
