...
class MuJoCoSimulation:
    def __init__(self,
        ...
        # 站立
        dof_list = [0., 0., 0.,  # waist_z/x/y 
                    0., 0., 0.,  0. , 0, 0, 0, # left_shoudler_y/x/z left_elbow left_wrist_z/y/x
                    0., 0., 0.,  0. , 0, 0, 0, # right_shoudler_y/x/z right_elbow right_wrist_z/y/x
                    0., 0., 0., 0.,  # left_hip_y/x/z left_knee
                    0., 0., # left_ankle_y/x
                    0., 0., 0., 0.,  # right_hip_y/x/z right_knee
                    0., 0., # right_ankle_y/x
                    ]
        self.arr_pos = np.array( [0.,  0.  , 0.9], dtype=np.float64)
        self.arr_quat = np.array([0.0, 0.0,  0.0,  1.], dtype=np.float64)

        # # 正面仰卧
        # dof_list = [0., 0., 0.,  # waist_z/x/y 
        #             0., 0.3, 0.,  1.57 , 0, 0, 0, # left_shoudler_y/x/z left_elbow left_wrist_z/y/x
        #             0., -0.3, 0.,  1.57 , 0, 0, 0, # right_shoudler_y/x/z right_elbow right_wrist_z/y/x
        #             0., 0., 0., 0.,  # left_hip_y/x/z left_knee
        #             0., 0., # left_ankle_y/x
        #             0., 0., 0., 0.,  # right_hip_y/x/z right_knee
        #             0., 0., # right_ankle_y/x
        #             ]
        # self.arr_pos = np.array( [0.,  0.  , 0.2], dtype=np.float64)
        # self.arr_quat = np.array([0.7071, 0.0,  -0.7071,  0.], dtype=np.float64)
    ...
    def start(self):
        ...
        # threading.Thread(target=self._send_robot_state, daemon=True).start()
    ...
    def _send_robot_state_thread(self):
        last_time = time.time()
        while True:
            if time.time() - last_time >= DT:
                last_time = time.time()
